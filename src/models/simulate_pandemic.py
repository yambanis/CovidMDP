...
def expose_population(pop_matrix, exposed, day):
    ...
    Returns:
        new_matrix (np.array): The population matrix with the newly exposed people exposed.
    Raises:
    ...
    if new_matrix.shape != pop_matrix.shape:
        raise ValueError("We lost someone during operations, shouldn't have happened...!")
    return new_matrix
...
def lambda_leak_expose(pop_matrix, day, lambda_leak = .0001):
    ...
    Returns:
        new_matrix (np.array): The population matrix with the newly exposed people.
    Raises:
        ValueError: If shape of starting matrix is different from final matrix
    """
    size = int(pop_matrix.shape[0]*lambda_leak)
    
    susceptible = pop_matrix[np.where(pop_matrix[:,1] == states_dict['susceptible'])][:,0]
    
    exposed = np.random.choice(susceptible, size=size, replace=False)
    
    if len(exposed) == 0: return pop_matrix
...
def spread_through_contacts(spreader, restrictions):
    """
    An infected person, or a spredear, infects each of its contacts with chance equal to 
    np.random.random() < p_r[r] * (1 - restrictions[r]).
    Returns an array of all the people successfully infected by the spreader.
    
    Args:
        spreader (int): id of the infected person that is spreading the disease.
            
        restrictions (dictionary): a dictionary with a value between zero and one for each type of relation
...
def update_population(pop_matrix):
    """
    Receives the population matrix and progress the infections for all people. The state duration is decremented
    ...
    Raises:
        ValueError: If shape of starting matrix is different from final matrix
    """
    matrix_keep = pop_matrix[np.isin(pop_matrix[:,1], [states_dict['susceptible'], states_dict['removed']])]
    matrix_change = pop_matrix[~np.isin(pop_matrix[:,1], [states_dict['susceptible'], states_dict['removed']])]
...
def spread_infection(pop_matrix, restrictions, day):
    """
    Receives the population matrix, the restrictions dictionary and the current day.
    The disease spreads throught the relations in the graph: each infected person has a chance to infect a
    susceptible contact with it has an edge in the graph, conditioned to the current restrictions, the p_r of
    each relations 
    ...
    """
    global G
    currently_infected = pop_matrix[np.where(pop_matrix[:,1] == states_dict['infected'])][:,0]

    if currently_infected.shape[0] == 0: return pop_matrix
    exposed = list(map(partial(spread_through_contacts, restrictions=restrictions), currently_infected))          
    exposed = np.unique(np.array([x for l in exposed for x in l]))
       
    mask = np.isin(pop_matrix[:,0], exposed)
    susceptible = np.isin(pop_matrix[np.array(mask)][:,1], states_dict['susceptible'])
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Run pandemic simulation for a given policy')
    parser.add_argument('policy', metavar='P', type=str, help='the policy to be used')
