...
class CovidState():
    ...
    def getPossibleRandomActions(self):
        possible_actions = [k for k in possible_policies.keys() 
                                    if self.current_policy - 2 <= k
                                     and k <= self.current_policy + 2 ]
        return possible_actions  
    ...
    def takeAction(self, action, step_size):
        ...
        for i in range(step_size):
            ...
            new_state.pop_matrix = simp.spread_infection(new_state.pop_matrix, possible_policies[action], new_state.day)
        ...
        new_state.rsieh = tuple(pd.Series(new_state.pop_matrix[:,1]).value_counts().sort_index())
        return new_state
    ...
    def getReward(self):
        return -np.sum(np.exp(self.cost_of_policies)) - 10e6 * self.days_over_capacity
